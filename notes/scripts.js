...
//Su ambiente de variables, el objeto 'this' que apunta a diversos lugares dependiendo de cómo se ejecute y referencias al ambiente de variables externo a la función
...
if (true && "non empty string" && 1 && -38748 && 'false') {
    ...
    //Toda esa vaina sigue siendo True. Al menos el type coercion no se mete con 'false' pa' volverlo false.
}
...
function brackets1() {
    ...
    // Esto retorna undefined, porque JS al llegar al return y ver que no hay nada después le pone una coma: "return;", por lo que se ejecuta 
...
function defaultvalue(value) {
    value = value || "Valor por default"; //Por la misma lógica del OR anterior (creo), si ve que el primero es vacío o undefines (false en JS), va al segundo y devuelve ese
    console.log(value);
}
...
//Con brackets uno tambien puede definir atributos, justo como en Python, por ende tambien puedes definir atributos con espacios así, con los puntos no se puede. ej: 
objeto["propiedad con espacios"] = "something random";
...
enviromentFunc;//Va a imprimir que el ambiente de la función es el ambiente interno, y si te das cuenta, se invoca de forma distinta
console.log(enviromentFunc);//Esto imprime un objeto, bien chido. Pero no puede retornar nada, remember dad

//Ejemplo: FUNCTION CONSTRUCTORS ---------->
...
//
//
//HTML viene con una serie de eventos predefinidos (como onClick, onBlur, onDrag, etc), que se ponen dentro de las etiquetas y a los que se les puede asignar una
//función de JavaScript para que los maneje EJ:
//<button onClick = "handlerFunction()"></button>
//Lo malo de esto es que "ensucia" el HTML y lo llena de eventos. Por lo que hay una formaa de hacerlo limpiamente desde JavaScript

//UNOBSTRUSIVE EVENT HANDLIG: Es la forma limpia de asignar eventos a una etiqueta HTML
...
document
    .querySelector("h1")
    .addEventListener("click", eventHandlerFunction);//Primera forma, es el nombre del evento pero sin el "on" que se le pone en HTML
document.querySelector("button")
    .onclick = eventHandlerFunction; //Segunda forma. La misma vaina que el de arriba

// ----OJO----------
//Si la función manejadora del evento se llama a partir de un evento definido directamente en HTML, "this" va a apuntar al objeto window.
//Si se llama usando JS con alguno de los dos métodos anteriores, "this", va a apuntar al elemento HTML que devuelve el querySelector
//----------OJO 2.0-----------
//Como puedes ver, los eventos son propiedades de los elementos de HTML, pero hay diferentes clases elementos y cada uno tiene su set de eventos
//En general, son los eventos de ventana, los eventos de documento, eventos de teclado, eventos de mouse, eventos de Media
//Eventos de drag, eventos Misc (que es uno solo, el OnToggle) y por último eventos globales
document.addEventListener("DOMContentLoaded", function(event){/*Do something*/}); //Bien usefull para tener código que corra solo después que se cargue la ventana

//Más info en: https://developer.mozilla.org/es/docs/Web/API/Window
//Y: https://www.w3schools.com/js/js_htmldom_eventlistener.asp
//Por último: https://www.w3schools.com/tags/ref_eventattributes.asp
//Leer todas las anteriores, o al menos ojear, todas tiene cosas distintas
...
//------------------------------------------------------------------------------------------------------------//
//
//Cargarlas primero que el sript principal en tu HTML, para que esté disponible
//Abajo una pequeña librería self made que hace GET request y recibe como parámetro la url del servidor y la función a llamar cuando se obtenga
//la respuesta. Para más info del objeto XMLHttpRequest (que es el que hace toda la magia) acá: https://developer.mozilla.org/es/docs/Web/API/XMLHttpRequest

(function (global) {
    var ajaxUtils = {};
    function getRequestObject() { // Esta fuención verifica que AJAX  esté disponible y devuelve una nueva XMLHttpRequest si solo está
    ...
    ajaxUtils.sendGetRequest = function (requestUrl, responseHandler) {
        ...
        request.onreadystatechange = function () {
            hnadleResponse(request, responseHandler);
        };
        request.open("GET",requestUrl, true);
        request.send(null); //Null porque es un GET, adentro va el body
    };

    function handleRepsonse(request, responseHandler) { //Verfica que la respuesta sea exitosa
...
//JSON 
var obj = JSON.parse('{"String with a json formatting": value}');
var string = JSON.stringify(obj); //Common JS onject



//Falta aún Hoisting, Promises y Arrow Functions, binding y toda esa vaina. Qué asquito
//Hoisting source: https://scotch.io/tutorials/understanding-hoisting-in-javascript